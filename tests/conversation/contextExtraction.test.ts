...
assert.ok(context.pressureSources.includes("schooldruk"));
assert.ok(context.underlyingNeeds.includes("richting"));
assert.ok(context.narrativeThemes.includes("alles alleen dragen"));
assert.equal(context.centralQuestion, "Kan ik mijn kind helpen zonder hier zelf aan onderdoor te gaan?");
